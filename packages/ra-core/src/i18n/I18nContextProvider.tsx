...
export const I18nContextProvider = ({
  ...
  useEffect(() => {
    if (locale && value.getLocale() !== locale) {
      new Promise(resolve => {
        // so we systematically return a Promsie for the messages
        // i18nProvider may return a Promise for language changes,
        resolve(value.changeLocale(locale));
      })
        .then(() => {
          // Force full page re-render
          // this is slow on locale change, but it's good way
          // to avoid having one subscription to the locale for each call to translate()
          setKey(key => key + 1);
          setInitialized(true);
        })
        .catch(error => {
