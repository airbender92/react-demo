...
export interface ResourceDefinition {
    ...
    readonly icon?: any;
    readonly reacordRepresentation?:
        | ReactElement
...
export type RenderResourcesFunction = (permissions: any) =>
    | ReactNode // (permissions) => <><Resource /><Resource /></>
    | Promise<ReactNode> // (promissions) => fetch().then(() => <><Resource/></Resource>)
    | ResourceElement[] // (permissions) => [<Resource />, <Resource />]
    | Promise<ResourceElement[]>; // (permissions) => fetch().then(() => [<Resource />])
